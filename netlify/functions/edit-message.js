...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (!messageId || typeof newText !== 'string' || newText.trim() === '') {
            return { statusCode: 400, headers, body: JSON.stringify({ error: 'Message ID and non-empty new text are required.' }) };
        }
        ...
        if (messageDoc.data().username !== userEmail) {
            return { statusCode: 403, headers, body: JSON.stringify({ error: 'Forbidden: You can only edit your own messages.' }) };
        }
        ...
        await pusher.trigger('chat-channel', 'message-edited', {
            id: messageId,
        ...
        console.error("SERVER ERROR [edit-message]:", error);
